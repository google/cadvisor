...
func RegisterHandlers(mux httpmux.Mux, containerManager manager.Manager, httpAuthFile, httpAuthRealm, httpDigestFile, httpDigestRealm string, redirectPrefix string) error {
	...
	mux.Handle("/", http.RedirectHandler(redirectPrefix + pages.ContainersPage, http.StatusTemporaryRedirect))
	...
	if httpAuthFile != "" {
		...
		if err := pages.RegisterHandlersBasic(mux, containerManager, authenticator, redirectPrefix); err != nil {
	...
	if httpAuthFile == "" && httpDigestFile != "" {
		...
		mux.HandleFunc(static.StaticResource, authenticator.Wrap(staticHandler))
		if err := pages.RegisterHandlersDigest(mux, containerManager, authenticator, redirectPrefix); err != nil {
	...
	if !authenticated {
		...
		if err := pages.RegisterHandlersBasic(mux, containerManager, nil, redirectPrefix); err != nil {
