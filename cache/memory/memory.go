...
func (c *Cache) AddStats(cInfo *info.ContainerInfo, stats *info.ContainerStats) error {
...
func (c *Cache) RecentStats(name string, start, end time.Time, maxStats int) ([]*info.ContainerStats, error) {
	var cstore *containerCache
	var ok bool
	err := func() error {
		c.lock.RLock()
...
func (c *Cache) RemoveContainer(containerName string) error {
	c.lock.Lock()
