...
`machine_node_hugepages_count` | Gauge |  Number of hugepages assigned to NUMA node | | cpu_topology |
`machine_node_memory_capacity_bytes` | Gauge |  Amount of memory assigned to NUMA node | bytes | cpu_topology |
`machine_node_numastat_pages` | Gauge | Number of memory hit/miss statistics per NUMA node. | | node_numastat |  
`machine_node_vmstat_pages` | Gauge | Number of virtual memory hit/miss statistics per NUMA node. | | node_vmstat |  
